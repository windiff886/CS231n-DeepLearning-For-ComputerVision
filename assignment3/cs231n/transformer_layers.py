...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        exp = torch.arange(0, embed_dim, 2) / embed_dim  # (embed_dim/2,)
        div_term = 10000 ** exp
        pe[0, :, 0::2] = torch.sin(pos / div_term)  # sin for even indices
        pe[0, :, 1::2] = torch.cos(pos / div_term)  # cos for odd indices
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
