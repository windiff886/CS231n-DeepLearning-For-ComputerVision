...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        cap_words = self.embedding(captions)  # (N, T, W)
        cap_words = self.positional_encoding(cap_words)  # (N, T, W)
        tgt_mask = torch.tril(torch.ones((T, T), device=features.device))  # (T, T)
        img_features = self.visual_projection(features)  # (N, W)
        scores = self.transformer(cap_words, img_features.unsqueeze(1), tgt_mask=tgt_mask)  # (N, T, W)
        scores = self.output(scores)  # (N, T, V)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        input = self.patch_embed(x)  # (N, num_patches, embed_dim)
        input = self.positional_encoding(input)  # (N, num_patches, embed_dim)
        encoded = self.transformer(input)  # (N, num_patches, embed_dim)
